...
`qwen3.5-4b-vl-dflash-multimodal-npu.yaml`); the remaining catalog is
text-only.
...
| `model.use_liger_kernel` | `false` | Patch in Liger fused Qwen3 RMSNorm/SwiGLU kernels for `training.strategy=dflash` (requires the `specforge[liger]` extra). |
